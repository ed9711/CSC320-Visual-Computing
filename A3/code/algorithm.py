...
def propagation_and_random_search(source_patches, target_patches,
    ...
    #############################################
    a, b ,c ,d = source_patches.shape[0], source_patches.shape[1], source_patches.shape[2], source_patches.shape[3]

    if best_D is None:
        target = f + make_coordinates_matrix([f.shape[0], f.shape[1]])
        print target.shape
        target = target_patches[target[..., 0], target[..., 1]]
        square_diff = (source_patches - target) ** 2
        # diff_sums = np.nansum(np.nansum(square_diff, axis=2), axis=2)
        diff_sums = np.nansum(square_diff.flatten())
        best_D = np.sqrt(diff_sums)
