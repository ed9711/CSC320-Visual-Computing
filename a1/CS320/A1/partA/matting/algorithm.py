...
"""
        ...
        #########################################
        problem = False
        pointers = ["alphaIn", "colIn", "backIn"]
        img_pointers = map(lambda key: self._images[key], pointers)
        problem = (img_pointers is None).any()
        if problem:
            msg = msg.format(status="unsuccessful, missing pixels")
            success = False
        else:
            try:
                foreground = self._images[pointers[1]]
                foreground_alpha = self._images[pointers[0]]
                background = self._images[pointers[2]]

                composite = foreground + (np.matmul(background, 1 - foreground_alpha))
                self._images["compOut"] = np.clip(composite, 0, 1)
                success = True
                msg = msg.format(status="successful")

            except Exception:
                msg = msg.format(status="unsuccessful in compositing")
                success = False
        #########################################
